...
/* #endif */
 ...
 int zeta_impl_series(float *a, float *b, float *s, const float x,
                     const float machep) {
  int i = 0;
  while (i < 9) {
    i += 1;
 ...
 int zeta_impl_series_dbl(double *a, double *b, double *s, const double x,
                         const double machep) {
  int i = 0;
  while ((i < 9) || (*a <= 9.0)) {
    i += 1;
 ...
 float polynomial_evaluation(float x, const float *f, int n) {
  float result = 0.0;
  for (int i = 0; i < n; i++) {
    result *= x;
    result += f[i];
 ...
 double polynomial_evaluation_dbl(double x, const double *f, int n) {
  double result = 0.0;
  for (int i = 0; i < n; i++) {
    result *= x;
    result += f[i];
 ...
 float digamma_impl_maybe_poly(const float s) {
 ...
 double digamma_impl_maybe_poly_dbl(const double s) {
 ...
 float digamma_impl(const float u) {
  float x = u;
  float p, q, nz, s, w, y;
  bool negative;
 ...
 double digamma_impl_dbl(const double u) {
  double x = u;
  double p, q, nz, s, w, y;
  bool negative;
